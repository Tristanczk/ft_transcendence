...
export class GamesController {
	...
	@UseGuards(JwtGuard)
	@Post('init')
	async initGame(@GetUser('id') userId: number, @Body() body: InitGameDto) {
		return this.gamesService.initGame(userId, body.idPlayerB, body.mode);
	}

	@UseGuards(JwtGuard)
	@Put(':id')
	async updateGame(@Param('id', ParseIntPipe) idGame: number, @Body() body: updateGameDto) {
