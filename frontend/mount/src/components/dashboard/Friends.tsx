...
function Friends({user, userList}: FriendsProps) {
	...
	const [possibleFriendsList, setPossibleFriendsList] = useState<UserSimplified[]>([]);
				...
				<div className='mb-6'>
					...
					<form>
						...
						<select name="users" id="users" onChange={handleChangeListChooseFriend}>
							...
							{possibleFriendsList.map((user) => (<option value={user.id} key={user.id}>{user.nickname}</option>))}
						</select>
						<button 
